...
# session-bootstrap.sh — SessionStart hook for local-review-kit
#
...
PLUGIN_DATA="${HOME}/.claude/plugins/data/local-review-kit"
...
format_cached_success() {
    local hash="$1"
    printf '%s' "local-review-kit -> ok (cached)"
}
...
format_full_success_user() {
    ...
    if [ ${#steps[@]} -eq 0 ]; then
        printf '%s' "local-review-kit -> bootstrapped (system tools, venv, git deps)"
        return
    fi

    local detail
    detail="$(IFS='; '; printf '%s' "${steps[*]}")"
    printf '%s' "local-review-kit -> bootstrapped: ${detail}"
}

format_full_success_agent() {
    printf '%s' "local-review-kit -> ok (validated: system tools, venv, git deps, config)"
}
...
format_bootstrap_error_context() {
    ...
    if [ -n "$context_msg" ]; then
        ...
        printf '%s' "local-review-kit -> Bootstrap failed. Failures (fix in this order):
${decoded}
'fix-all' means fix each failure in the order listed above. After all fixes succeed, tell the user to restart Claude Code so bootstrap can verify the changes."
        ...
        printf '%s' "local-review-kit -> ERROR: $msg"
    fi
}
...
format_bootstrap_error_user() {
    ...
    if [ -n "$user_msg" ]; then
        ...
        printf '%s' "local-review-kit -> Setup issues found:
${decoded}"
    else
        local msg
        msg="$(_extract_json_field "$step_json" "message")"
...
format_config_error_context() {
    ...
    if [ -n "$context_msg" ]; then
        local decoded
        decoded="$(printf '%b' "$context_msg")"
        printf '%s' "local-review-kit -> Config setup needed:
${decoded}"
    else
        local msg
        msg="$(_extract_json_field "$step_json" "message")"
        printf '%s' "local-review-kit -> Config ERROR: $msg"
    fi
}
...
format_config_error_user() {
    ...
    if [ -n "$user_msg" ]; then
        local decoded
        decoded="$(printf '%b' "$user_msg")"
        printf '%s' "local-review-kit -> ${decoded}"
    else
        local msg
        msg="$(_extract_json_field "$step_json" "message")"
        printf '%s' "local-review-kit -> Config ERROR: $msg"
