...
agent_glue_lib/claude_work_queue/
  __init__.py           # exposes submit(), wait(), result(), typed errors
  storage.py            # read/write WorkItem and WorkItemResult yamls against <queue_root>/
  claim.py              # atomic claim via os.rename (pending/ -> in_progress/)
  report.py             # result write + schema validation + done/ promotion
```
...
| Topic | Document |
|---|---|
| The queue + signal + execute-and-report primitive; the three locked design decisions (file-based storage, Stop-hook signaling, open-to-any-writer scope); WorkItem and WorkItemResult wire format | DESIGN.md |
| Entities + components (WorkItem, WorkItemResult; authored in the first implementation increment) | ARCHITECTURE.md |
| Build increments and acceptance criteria | IMPLEMENTATION-PLAN.md |
| Entity-type definitions and component schemas | entities/ and components/ |
