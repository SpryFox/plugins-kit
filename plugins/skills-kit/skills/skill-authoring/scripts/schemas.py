            ...
            "items": {
                "keys": {
                    ...
                    "checklist": {"type": "list", "required": False,
                                  "note": "Dec-8: when the technique has >3 steps, explicit step-tracking is required, satisfied by EITHER this paste-able checklist field OR an explicit step-tracker invocation (TaskCreate, scratch file, etc.) in the steps body. The schema does not encode the conditional rule directly; audit.py enforces the OR-form on the rendered SKILL.md body."},
                },
            },
        },
...
CAPABILITY_SKILL_SCHEMA = {
    ...
    "keys": {
        ...
        "subdomain_config": {"type": "list", "required": False,
            "note": "optional per-sub-area configuration for capability-skills with multiple sub-areas; each entry contracts the agent's and designer's shared vocabulary for one sub-area",
            "items": {"keys": {
                "name": {"type": "string", "required": True,
                         "note": "canonical sub-area identifier; matches the sub-domain registration index"},
                "state_terms": {"type": "list", "required": False,
                                "note": "canonical state vocabulary used verbatim; agent re-phrases user-loose language into these terms"},
                "operations": {"type": "list", "required": False,
                               "note": "verbs the sub-area supports; one verb (or short verb phrase) per entry"},
                "scope_axes": {"type": "list", "required": False,
                               "note": "axes that decompose the capability space within the sub-area; each entry typically a {name, values} dict"},
                "canonical_phrasing": {"type": "string", "required": False,
                                       "note": "readback rule restating user requests in canonical vocabulary with placeholders"},
                "llm_dependent_content": {"type": "list", "required": False,
                                          "note": "fields whose values are LLM-produced rather than retrieved deterministically"},
                "dependency_order": {"type": "list", "required": False,
                                     "note": "ordering constraints among the sub-area's capabilities; each entry names a constraint"},
            }},
        },
    },
    "forbidden_keys": ["rules", "counters", "facts", "patterns",
