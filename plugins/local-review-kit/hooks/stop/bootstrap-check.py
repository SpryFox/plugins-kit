...
def _resolve_paths():
    ...
    plugin_data = os.path.join(
        os.path.expanduser("~"), ".claude", "plugins", "data", "local-review-kit"
    )
    ...
    if os.path.isfile(setup_script):
        ...
        if not config_ok:
            reason = (
                ...
                "local-review-kit configuration is missing or incomplete.\n\n"
                "Say **'setup local-review-kit'** to run the interactive setup, "
                "or invoke the `local-review-setup` skill."
            )
            print(json.dumps({"decision": "block", "reason": reason}))
            return
